...
export class JdColrowGroup {
  private key!: RowKey;
  private tick!: NodeJS.Timeout | number;
  private stateSubject: Subject<GroupState> | null = null;
  ...
  init() {
    this.stateSubject = new Subject<GroupState>();
  }
  ...
  destory() {
    try {
      cancelAnimationFrame(this.tick as number);
